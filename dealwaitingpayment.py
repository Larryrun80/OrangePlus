...
def get_not_dealed_wechat_orders():
    ...
    '''
    # 取出所有状态为19 的订单
    result = db_wechat_handler.execute(
        querystring_get_wechat_error_order).fetchall()
...
def find_distirbution_no(pool_id):
    ...
    SELECT ecode
     ...
     LIMIT 1;
    '''
    ecode = db_ecode_handler.execute(
        querystring_get_ecode, pool_id).fetchall()[0][0]
...
try:
    ...
    if len(result) == 0:
        ...
        for (order_id, pool_id) in result:
            logger.info(
                'Dealing With Order: %s, Which With Pool %s', order_id, pool_id)
            # 更新一个可用的验证码状态，并取出这个验证码
            ecode = find_distirbution_no(pool_id)
            logger.info(
                'Dealing With Order: %s, Using Distribution No %s', order_id, ecode)
            # 更新订单
