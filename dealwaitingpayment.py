...
def get_not_dealed_wechat_orders():
    ...
    '''
    # 取出所有状态为19 的订单
    result = db_wechat_handler.execute(qs_wechat_error_order).fetchall()
...
def get_order_info(type, value):
    ...
    '''
    if type == 'sn':
        qs_get_order_info = qs_get_order_info +\
            '''
              AND o.orderSn = %s
              AND o.orderStatus in (-2,-1,0)
            '''
    if type == 'status':
        qs_get_order_info += ' AND o.orderStatus = %s'
    return db_orange_handler.execute(qs_get_order_info, value).fetchall()
...
def find_distirbution_no(pool_id):
    ...
    SELECT ecode
     ...
     LIMIT 1;
    '''
    ecode = db_ecode_handler.execute(qs_get_ecode, pool_id).fetchall()[0][0]
    qs_set_ecode_status = '''
    UPDATE p%s
              SET isUsed = 1
     WHERE ecode =%s
           AND isUsed = 0;
    '''
    db_ecode_handler.execute(qs_set_ecode_status, pool_id, ecode)
...
def update_order(order_id, ecode):
    ...
    UPDATE murcielago_order
     ...
     WHERE orderId =%s ;
    '''
    db_orange_handler.execute(qs_update_order, ecode, order_id)
...
try:
    ...
    if len(result) == 0:
        ...
        for (order_id, pool_id) in result:
            logger.info('Dealing With Order: %s, Which With Pool %s',
                        order_id, pool_id)
            # 更新一个可用的验证码状态，并取出这个验证码
            ecode = find_distirbution_no(pool_id)
            logger.info('Dealing With Order: %s, Using Distribution No %s',
                        order_id, ecode)
            # 更新订单
